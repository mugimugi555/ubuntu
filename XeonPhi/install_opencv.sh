...
### 1️⃣ Xeon Phi に OpenCV をインストール（なければビルド） ###
echo "=== Xeon Phi の OpenCV 確認 ==="

# Xeon Phi に OpenCV があるか確認
ssh mic0 'if ! command -v python3 &> /dev/null || ! python3 -c "import cv2" 2>/dev/null; then echo "OpenCV が見つかりません。インストールします..."; exit 1; else echo "OpenCV は既にインストール済み"; exit 0; fi'
if [ $? -eq 1 ]; then
    echo "=== OpenCV のインストールを開始 ==="
    ...
    # OpenCV のソースコードを取得 & ビルド
    ssh mic0 'git clone https://github.com/opencv/opencv.git && git clone https://github.com/opencv/opencv_contrib.git'
    ssh mic0 'mkdir -p opencv/build && cd opencv/build && cmake -D CMAKE_BUILD_TYPE=RELEASE \
...
ssh mic0 "python3 /home/mic/process_video.py"
